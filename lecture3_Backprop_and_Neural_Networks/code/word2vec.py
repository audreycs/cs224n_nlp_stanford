...
def naiveSoftmaxLossAndGradient(
    ...
    p_all = softmax(np.matmul(outsideVectors, centerWordVec))[:, np.newaxis]

    loss = float(-np.log(p_all[outsideWordIdx]))

    trueOutsideVec = outsideVectors[outsideWordIdx]
    gradCenterVec = - trueOutsideVec + np.matmul(p_all.transpose(), outsideVectors).squeeze()

    gradOutsideVecs = np.dot(p_all, centerWordVec[:, np.newaxis].T)
...
def negSamplingLossAndGradient(
    ...
    nei_vec = outsideVectors[outsideWordIdx]
    neg_vec = outsideVectors[negSampleWordIndices]
    p_o = sigmoid(np.matmul(nei_vec.transpose(), centerWordVec))
    p_k = sigmoid(- np.matmul(neg_vec, centerWordVec)[:, None])

    loss = float(- np.log(p_o) - np.sum(np.log(p_k), axis=0))

    grad_vc_1 = -nei_vec * (1-p_o)
    grad_vc_2 = np.sum(neg_vec * (1-p_k), axis=0)
    gradCenterVec = grad_vc_1 + grad_vc_2

    grad_o = np.zeros(outsideVectors.shape)
    grad_o[outsideWordIdx] = -centerWordVec * (1-p_o)
    grad_o[negSampleWordIndices] += centerWordVec * (1-p_k)

    indexCount = np.bincount(indices)[:, None]
    for i in np.unique(indices):
        grad_o[i] *= indexCount[i]

    gradOutsideVecs = grad_o
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    outsideWordsIndex = np.array([word2Ind[i] for i in outsideWords])

    current_id = word2Ind[currentCenterWord]
    current_center_vec = centerWordVectors[current_id]

    for o_index in outsideWordsIndex:
        cur_loss, cur_gradCenterVecs, cur_gradOutsideVectors = word2vecLossAndGradient(current_center_vec, o_index, outsideVectors, dataset)
        loss += cur_loss
        gradCenterVecs += cur_gradCenterVecs
        gradOutsideVectors += cur_gradOutsideVectors

    gradCenterVecs[np.arange(gradCenterVecs.shape[0]) != current_id] = 0
...
if __name__ == "__main__":
    ...
    parser.add_argument('function', nargs='?', type=str, default='naiveSoftmaxLossAndGradient',

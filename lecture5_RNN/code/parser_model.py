...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = nn.Parameter(torch.FloatTensor(self.n_features * self.embed_size, self.hidden_size))
        nn.init.xavier_uniform_(self.embed_to_hidden_weight)
        # self.embed_to_hidden_bias = nn.Parameter(torch.zeros(self.hidden_size, requires_grad=True))
        self.embed_to_hidden_bias = nn.Parameter(torch.FloatTensor(self.hidden_size))
        self.embed_to_hidden_bias.data.uniform_()

        self.dropout = nn.Dropout(p=self.dropout_prob)

        self.hidden_to_logits_weight = nn.Parameter(torch.FloatTensor(self.hidden_size, self.n_classes))
        nn.init.xavier_uniform_(self.hidden_to_logits_weight)
        self.hidden_to_logits_bias = nn.Parameter(torch.FloatTensor(self.n_classes))
        self.embed_to_hidden_bias.data.uniform_()
    ...
    def embedding_lookup(self, w):
        ...
        temp = self.embeddings.data[w]
        x = temp.view(w.shape[0], w.shape[1] * self.embed_size)
    ...
    def forward(self, w):
        ...
        h = self.dropout(h)
        I = torch.matmul(h, self.hidden_to_logits_weight) + self.hidden_to_logits_bias
        logits = F.softmax(I)

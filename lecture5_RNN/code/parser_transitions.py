...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == 'S' and len(self.buffer) > 0:
            ...
            tail = self.stack[len(self.stack)-2]
            self.dependencies.append((head, tail))
            self.stack = self.stack[:-2] + [head]
        else:
            head = self.stack[len(self.stack)-2]
            tail = self.stack[len(self.stack)-1]
            self.dependencies.append((head, tail))
            self.stack.pop()
...
def minibatch_parse(sentences, model, batch_size):
    ...
    pp_list = [ PartialParse(s) for s in sentences]

    un_pp_list = pp_list.copy()

    while un_pp_list.__len__() > 0:
        size = max(un_pp_list.__len__(), batch_size)
        batch = un_pp_list[0:size]
        trans_list = model.predict(batch)

        for p, t in zip(batch, trans_list):
            dependencies.append(p.parse([t]))
